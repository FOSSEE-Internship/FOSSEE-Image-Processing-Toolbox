...
extern "C"
{
    ...
    int opencv_predictBoost(char *fname, unsigned long fname_len)
    {
	...
	try{
        ...
        //sciprint("hello");
         ...
         //sciprint("hello1");
         dt->predict(features,response,1);
        sciprint("\ndone");
        }
        catch(cv::Exception& e)
	{
        ...
        sciErr = createMatrixOfString(pvApiCtx, nbInputArgument(pvApiCtx)+1, 1, 1, &classifierDescription[response.at<int>(0)]);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
