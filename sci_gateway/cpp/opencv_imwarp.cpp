/********************************************************
    Authors: V Srinivas , M Avinash Reddy
*********************************************************
...
extern "C"
{
  ...
  int opencv_imwarp(char *fname, unsigned long fname_len)
  {
    ...
    {
        modx[i]=(int)(tform[0] * x[i] + tform[2] * y[i] + tform[4]);
        mody[i]=(int)(tform[1] * x[i] + tform[3] * y[i] + tform[5]);
        //cout<<*(modx+i)<<" "<<*(mody+i)<<endl;
    }
    Mat trans=(Mat_<double>(2,3)<<tform[0],tform[2],tform[4],tform[1],tform[3],tform[5]);//creating a matrix of type Mat.
