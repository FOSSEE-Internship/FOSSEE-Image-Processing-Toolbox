...
extern "C"
{
    ...
    #include "sciprint.h"
    #include "../common.h"
    int opencv_getParamsANN(char *fname, unsigned long fname_len)
    {
       ...
       // char *classifierLocation = NULL;
        ...
        int descriptionCount;
         ...
         double bpms,bws,d0,d1,d2,d3,d4,nmeth;
         int lid =1;
...
//------Actual processing------//
	try{
       Ptr<ANN_MLP> ann = Algorithm::load<ANN_MLP>(classifierLocation);
        if(!ann->isTrained())
            {
        ...
        bpms=ann->getBackpropMomentumScale();
        bws=ann->getBackpropWeightScale();
        //layermat=ann->getLayerSizes();
        d0 = ann->getRpropDW0();
        d1 = ann->getRpropDWMax();
        d2 = ann->getRpropDWMin();
        d3 = ann->getRpropDWMinus();
        d4 = ann->getRpropDWPlus();
        nmeth = ann->getTrainMethod();
        ...
        catch(cv::Exception& e)
	{
       ...
       //------Create output arguments------//
        ...
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 1, 1, 1, &bpms);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 2, 1, 1, &bws);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
       ...
       /*sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 3, 1, 1, &layermat);
        ...
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 3, 1, 1, &d0);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
	sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 4, 1, 1, &d1);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
	sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 5, 1, 1, &d2);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
	sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 6, 1, 1, &d3);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
	sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 7, 1, 1, &d4);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 8, 1, 1, &nmeth);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
