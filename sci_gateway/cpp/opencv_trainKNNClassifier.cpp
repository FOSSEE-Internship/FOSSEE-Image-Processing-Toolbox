...
extern "C"
{
    ...
    int opencv_trainKNNClassifier(char *fname, unsigned long fname_len)
    {
        ...
        char *classifierLocation;
        char *classifierName = NULL;
        char *objectType = "KNNClassifier";
        ...
        // Check is imageSet is in its specified location.
        ...
        // Check is bagOfFeatures.yml is in its specified location.
			...
			if(isIntegerType(pvApiCtx, piAddr) || isDoubleType(pvApiCtx, piAddr))
			{
				...
				if(isScalar(pvApiCtx, piAddr))
				{
					...
					if(!iRet)
					{
                        if(dData < 1 || dData > 2)
                        {
                            Scierror(999, "Error: The input argument ""algorithmType"" should be in range [1, 2].\n");
                            return 0;
                        }
						algorithmType = dData;
					}
        ...
        try
        {
        	knearest->setAlgorithmType(algorithmType);	
        }
        catch(cv::Exception &e)
        {
        	const char *err = e.what();
        ...
        try
        {	
        	knearest->train(trainingData, ml::ROW_SAMPLE, labels);
		...
		try
        {
        	knearest->save(classifierLocation);
        }
        catch(cv::Exception &e)
        {
        	const char *err = e.what();
