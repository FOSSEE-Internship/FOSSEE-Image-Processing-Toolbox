...
extern "C"
{
    ...
    #include "sciprint.h"
    #include "../common.h"
    int opencv_getParamsdtree(char *fname, unsigned long fname_len)
    {
       ...
       // char *classifierLocation = NULL;
        ...
        int descriptionCount;
         ...
         double cv_f,max_cat,md,msc,reg_acc;
...
//------Actual processing------//
	try{
       Ptr<DTrees> dt = Algorithm::load<DTrees>(classifierLocation);
        if(!dt->isTrained())
            {
	...
	int prune1 = prune;
       ...
       //------Create output arguments------//
        ...
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 1, 1, 1, &cv_f);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 2, 1, 1, &max_cat);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
       ...
       /*sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 3, 1, 1, &layermat);
        ...
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 3, 1, 1, &md);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        }
	sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 4, 1, 1, &msc);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
	...
	sciErr = createMatrixOfBooleanInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 7, 1, 1, &use1_se1);
        ...
        sciErr = createMatrixOfBooleanInList(pvApiCtx, nbInputArgument(pvApiCtx)+1, piAddr, 8, 1, 1, &use_surr1);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
