...
//	PROCESSING BRISK
        ...
        Ptr<FeatureDetector> myobject = BRISK::create();
        try 
        {
...
//	LOCATION   ---->
    ...
    for(int i=0;i<size;i++)
    {
    	if(myPoints[i].response>=minMetric)
    	{	        
	        validKeyPoints.push_back(myPoints[i]);
	        OrientationData[i] = 0;    
    ...
    int validKeyPointsSize = validKeyPoints.size();
    LocationData = (double *)malloc(sizeof(double) * validKeyPointsSize * 2);
    
    for(int i = 0; i < validKeyPointsSize; i++)
    {
        LocationData[i] = myPoints[i].pt.x;
        LocationData[i + validKeyPointsSize] = myPoints[i].pt.y;
    }
