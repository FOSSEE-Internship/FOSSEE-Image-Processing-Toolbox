/********************************************************
    Author: Sukul Bagai, Ebey Abraham
*********************************************************
   return_image = rectangle(input_image , x_coordinate_of_point1 , y_coordinate_point1, x_coordinate_of_point2 , y_coordinate_point2 , r_value,g_value,b_value,thickness,linetype,shift);
********************************************************/
	...
	int opencv_rectangle(char *fname, unsigned long fname_len)
	{
		...
		try
		{
			...
			if (sciErr.iErr)
		   {
			...
			{
				r_value=0;
 				Scierror(999,"r value of colour should be between 0 and 255 , using 0 instead\n");
			}
			if( g_value <0 || g_value >255)
			{
				g_value=0;
				Scierror(999,"g value of colour should be between 0 and 255 , using 0 instead\n");
			}
			if( b_value <0 || b_value >255)
			{
				b_value=0;
				Scierror(999,"b value of colour should be between 0 and 255 , using 0 instead\n");
			}
			if ((linetype!=0) && (linetype!=4)&& (linetype!=8))
			{
				linetype=8;
				Scierror(999,"Only 0/4/8 allowed , using 8 instead\n");
			}

			// Draw the circles detected
			rectangle( src, Point(point1_x,point1_y), Point(point2_x,point2_y), Scalar(b_value,g_value,r_value), thickness, linetype, shift);
