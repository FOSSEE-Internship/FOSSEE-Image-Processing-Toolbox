...
//*********************************************  End of OIA  ********************************************************
    ...
    Ptr<SURF> detector = SURF::create(MetricThreshold, NumOctaves, nOctaveLayers, 1, false);

    // my_object.operator()(image, mask, myPoints, noArray());
    try
    {   
        image.convertTo(image, CV_8UC1);
        detector->detect(image, myPoints);    
    }
    catch(Exception &e)
    {
...
//*************************************************************** Location ***************************************************************
	 ...
	 OrientationData = (double *)malloc(sizeof(double)*size);
 	 ...
 	 LaplacianData = (double*)malloc(sizeof(double)*size);
    ...
    {
        //sciprint("%f %f", myPoints[i].pt.x, myPoints[i].pt.y);
        LocationData[k] = myPoints[i].pt.x;
        LocationData[k + size] = myPoints[i].pt.y;
	    OrientationData[i] = 0;
    ...
    // for(int i = 0; i < size; i++)
    // {
    //     sciprint("%f %f", LocationData[i], LocationData[i + size]);
    // }
...
//************************************************************  SignOfLaplacian ***********************************************************
    ...
    sciErr = createMatrixOfDouble(pvApiCtx, nbInputArgument(pvApiCtx) + 4, size,1, LaplacianData);
    if(sciErr.iErr)
    {
        printError(&sciErr, 0);
        return 0;
