...
//**************************************************************Actual Processing*******************************************************************************
        ...
        vector <Point> slocations;
        
        d.detect( img_gray, locations,0, Size(8,8), Size(4,4), slocations);
        d.compute( img_gray, descriptorsValues, Size(16,16), Size(0,0), locations);
        ...
        //sciprint("%d",location_size);
        location = (double*) malloc(sizeof(double)*location_size*2); //doesn't have floating point
        for(int i=0; i<location_size; i++)
        {
            location[i] = locations[i].x;
            location[i+location_size] = locations[i].y;
        }
...
//************************************************************ Output **************************************************************************
        ...
        //sciprint("%d",featureVector_size);
        
        sciErr = createMatrixOfDouble(pvApiCtx, nbInputArgument(pvApiCtx)+2, location_size, 2, location); 
