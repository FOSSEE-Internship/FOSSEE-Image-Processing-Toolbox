...
extern "C"
{
    ...
    #include "sciprint.h"
    #include "../common.h"
    int opencv_predictLR(char *fname, unsigned long fname_len)
    {
        ...
        createMatrixOfStringInList(pvApiCtx, nbInputArgument(pvApiCtx) + 1, piAddr, 1, 1, 1, &classifierDescription[response.at<int>(0)]);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
