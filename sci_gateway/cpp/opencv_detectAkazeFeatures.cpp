...
extern "C"
{
	...
	int opencv_detectAkazeFeatures(char *fname, unsigned long fname_len)
	{	
...
//	ARGUMENT CHECK
		...
		CheckInputArgument(pvApiCtx, 1,17);
        ...
        for(int i=2;i<noOfarguments;i=i+2)
            {
                ...
                else if(strcmp(currentArg,"desctype")==0)
                {
                    ...
                    if(!isDoubleType(pvApiCtx, piAddrVal))
                    {
                        Scierror(999," Invalid type for desctype\n");
                        return 0;
                    }
                    
                    getScalarDouble(pvApiCtx, piAddrVal, &r);
                     
                     if(r>3 || r<0)
                    {
                        Scierror(999,"  desctype must be [0,3] \n");
                        return 0;
                ...
                else if(strcmp(currentArg,"desc_ch")==0)
                {
                    ...
                    getScalarDouble(pvApiCtx, piAddrVal, &r1);
                    
                     
                     if(r1<0)
                    {
                        Scierror(999,"  desc_ch must be >0 \n");
                        return 0;
                ...
                else if(strcmp(currentArg,"desc_size")==0)
                {
                    val_position=i+1;
                    ...
                    getScalarDouble(pvApiCtx, piAddrVal, &desc_size);
                    

                    if(diffusivity>3 || diffusivity<0)
                    {
                        Scierror(999,"  desc_size must be >0 \n");
                        return 0;
...
//KAZE processing
            Mat descriptor;
        Ptr<AKAZE> myobject = AKAZE::create(3,0,3,Threshold,NumOctaves,NumLayers,diffusivity);
        try 
        {
