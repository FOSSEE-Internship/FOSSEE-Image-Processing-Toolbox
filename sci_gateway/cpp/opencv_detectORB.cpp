...
extern "C"
{
    ...
    int opencv_detectORB(char *fname, unsigned long fname_len)
    {
        ...
        try
        {
            ...
            orb->detectAndCompute(image, keyPoints, mask, descriptor);
        }
        catch(Exception &e)
        {
            const char *err = e.what();
        ...
        double *_descriptors = (double*) malloc(sizeof(double) * dcols);
        
        for(int i = 0; i < dcols; i++)
        {
            _descriptors[i] = descriptor.at<double>(i);
        }
        ...
        sciErr = createMatrixOfDoubleInList(pvApiCtx, nbInputArgument(pvApiCtx) + 1, piAddr, 2, 1, dcols, _descriptors);
        if(sciErr.iErr)
        {
          printError(&sciErr, 0);
          return 0;
