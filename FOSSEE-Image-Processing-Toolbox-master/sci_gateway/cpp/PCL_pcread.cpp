...
bool files_exist_check( char *name) {
 ...
 int PCL_pcread(char *fname, unsigned long fname_len)
 {
    ...
    sciErr = createMatrixOfInteger32(pvApiCtx, nbInputArgument(pvApiCtx) + 3, 1, 1,&width);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
        } 
    sciErr = createMatrixOfInteger32(pvApiCtx, nbInputArgument(pvApiCtx) + 4, 1, 1,&height);
        if(sciErr.iErr)
        {
            printError(&sciErr, 0);
            return 0;
