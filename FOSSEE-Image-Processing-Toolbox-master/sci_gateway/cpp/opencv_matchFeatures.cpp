	...
	int opencv_matchFeatures(char *fname, unsigned long fname_len)
	{
		...
		if( strcmp(method[0],"Exhaustive") == 0)
		{
			...
			{	
				...
				try
				{
					//sciprint("\nd1:%d %d\n", descriptor_1.rows, descriptor_1.cols);
					//sciprint("\nd2:%d %d\n", descriptor_2.rows, descriptor_2.cols);
		...
		if(matches1.size() != 0)
		{
			...
			{
				//sciprint("%d %f %f\n", matches1[i].size(), matches1[i][0].distance, matches1[i][1].distance);
				if((matches1[i][0].distance < 0.9 * (matches1[i][1].distance)) && 
		...
		if(matches.size() != 0)
		{
			...
			{	
				//sciprint("%f  ", matches[i].distance);
				if((matches[i].distance < 0.2 * (matches[i + 1].distance)))
				{
