...
extern "C"
{
  ...
  int opencv_distortPoints(char *fname, unsigned long fname_len)
  {
    //variables
    int i,j,k,n,m;
    int iRows=0,iCols=0;
    ...
    /* ------------------------------------ Skew Coefficient :- Fourth argument of the input -------------------------------------*/
     sciErr = getVarAddressFromPosition(pvApiCtx,4,&piAddr4);
                    ...
                    getScalarDouble(pvApiCtx, piAddr4, &alpha);
                    if(alpha<0)
                    {
                        Scierror(999," Invalid Value for NumOctaves. Recommended values are between 0 and 4\n");
                        return 0;
                    }
    ...
    /* opencv function for distortPoints */
    cv::fisheye::distortPoints(Src, dst, cameraMat, distCoeffsActual, alpha);
